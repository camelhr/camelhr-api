...
func TestErrorResponse(t *testing.T) {
	...
	t.Run("should write empty error message to response for general error and log the error message",
		func(t *testing.T) {
			...
			response.ErrorResponse(rr, assert.AnError)
	...
	t.Run("should write error message to response but not log the error for api error without cause",
		func(t *testing.T) {
			...
			response.ErrorResponse(rr, base.NewAPIError("test error"))
	...
	t.Run("should write error message to response but not log the error for not-found error",
		func(t *testing.T) {
			...
			response.ErrorResponse(rr, base.NewNotFoundError("test not found error"))
	...
	t.Run("should write error message to response and log the cause when error has a cause",
		func(t *testing.T) {
			...
			// call the ErrorResponse function
			response.ErrorResponse(rr, base.NewAPIError("test error", base.ErrorCause(assert.AnError)))
	...
	t.Run("should write error message and set the provided status code",
		func(t *testing.T) {
			t.Parallel()
			...
			// call the ErrorResponse function
			response.ErrorResponse(rr, base.NewAPIError("test error", base.ErrorHTTPStatus(http.StatusTeapot)))
	...
	t.Run("should write error message to response and log the error when error is wrapped",
		func(t *testing.T) {
			...
			response.ErrorResponse(rr, base.WrapError(assert.AnError))
